...
export default function JoelhoCaptureScreen({ route, navigation }: any) {
  ...
  async function escolherDaGaleria() {
    ...
    if (!r.canceled && r.assets && r.assets[0]) {
      navigation.navigate('JoelhoMarking', { fotoUri: r.assets[0].uri, pacienteId, vista });
    }
  }
  ...
  if (!permission.granted) {
    return (
      <View style={styles.container}>
        <Text style={styles.text}>Precisamos de acesso a camera para a avaliacao de joelhos.</Text>
  ...
  const tirarFoto = async () => {
    if (cameraRef.current) {
      try {
        ...
        navigation.navigate('JoelhoMarking', { fotoUri: foto.uri, pacienteId, vista });
      } catch (error) {
