...
export async function gerarRelatorioJoelho(idAvaliacao: number) {
  ...
  // Cards de desvio marcados pelo terapeuta.
  let cardsHtml = '';
  if (av.cards_json) {
    try {
      const cards = JSON.parse(av.cards_json) as string[];
      if (cards.length > 0) {
        cardsHtml = '<div class="bloco"><b>Achados observados</b></div><ul>'
          + cards.map(c => '<li>' + c + '</li>').join('') + '</ul>';
      }
    } catch { cardsHtml = ''; }
  }

  // Achados clinicos complementares, com numero, nome e lado.
  let achadosHtml = '';
  if (av.achados_json) {
    try {
      const achados = JSON.parse(av.achados_json) as Record<string, string>;
  ...
  // Classificacao da pisada registrada pelo terapeuta.
  let pisadaHtml = '';
  if (av.pisada_json) {
    try {
      const pisada = JSON.parse(av.pisada_json) as Record<string, string>;
      const itens = Object.entries(pisada).filter(([, v]) => v !== '')
        .map(([lado, valor]) => '<li>' + lado + ': ' + valor + '</li>').join('');
      if (itens !== '') {
        pisadaHtml = '<div class="bloco"><b>Classificacao da pisada</b></div><ul>' + itens + '</ul>';
      }
    } catch { pisadaHtml = ''; }
  }

  const protocolo = vista === 'retrope'
    ? 'Paciente em pe, descalco, de costas para a camera, pes alinhados na largura do quadril '
      + 'e carga bipodal igual. Camera a 1 metro, na altura do tornozelo, perpendicular ao chao.'
    : 'Paciente em pe, descalco, pes alinhados na largura do quadril e carga bipodal igual. '
  ...
  const html = `
    <html><head><meta charset="utf-8">${ESTILO}</head><body>
      ...
      ${cardsHtml}
      ${achadosHtml}
      <div class="bloco"><b>Protocolo de captura</b><br/>${protocolo}</div>
      ${referencias}
      ${rodapeHtml}
    </body></html>
  `;
